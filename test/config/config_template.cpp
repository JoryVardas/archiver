...
TEST_CASE("Loading a complete config file", "[config]") {
  REQUIRE_NOTHROW(Config("./test_config.json"));
...
TEST_CASE("Loading a config file missing a single value", "[config]") {
  REQUIRE_THROWS_AS(Config("./test_config_incomplete1.json"), ConfigError);
  REQUIRE_THROWS_WITH(
    Config("./test_config_incomplete1.json"),
...
TEST_CASE("Loading a config file missing an entire section", "[config]") {
  REQUIRE_THROWS_AS(Config("./test_config_incomplete2.json"), ConfigError);
  REQUIRE_THROWS_WITH(Config("./test_config_incomplete2.json"),
...
TEST_CASE("Loading an empty config file", "[config]") {
  REQUIRE_THROWS_AS(Config("./test_config_empty.json"), ConfigError);
  REQUIRE_THROWS_WITH(
...
TEST_CASE("Loading a config file which doesn't exist", "[config]") {
  REQUIRE_THROWS_AS(Config("./test_config_non_existent.json"), ConfigError);
  REQUIRE_THROWS_WITH(Config("./test_config_non_existent.json"),
                      StartsWith("Could not open the configuration file"));
}

TEST_CASE("Loading a config file with an incorrect value type", "[config]") {
  REQUIRE_THROWS_AS(Config("./test_config_incorrect_type.json"), ConfigError);
  REQUIRE_THROWS_WITH(Config("./test_config_incorrect_type.json"),
